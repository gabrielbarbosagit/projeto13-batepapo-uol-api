...
app.post("/messages", async (req, res) => {
    try {
        ...
        if (!participant) {
            return res.status(422).json({ error: "Remetente não encontrado" });
        }
...
// Função para remover participantes inativos e salvar uma mensagem
async function removeInactiveParticipantsAndSaveMessage() {
    try {
        const db = client.db();
        ...
        console.error("Erro ao remover participantes inativos e salvar mensagem:", error);
    }
}

// Executar a remoção automática e salvamento de mensagem a cada 15 segundos
setInterval(removeInactiveParticipantsAndSaveMessage, 15000);
